...
class MertonJump_CompoundPoisson:
    ...
    def init_jumpMdl(self, _compPois_mdl: Agg_PoiFft):
        ...
        cf_jump_real = interp1d(
            Xi_jump, np.real(severity_cf), kind="linear", fill_value="extrapolate"
        )
        cf_jump_imag = interp1d(
            Xi_jump, np.imag(severity_cf), kind="linear", fill_value="extrapolate"
        )

        # Recombined complex interp - Severity CF on jump vec
        severity_cf_on_jump = cf_jump_real(np.real(Xi_jump)) + 1j * cf_jump_imag(
            np.real(Xi_jump)
        )
    ...
    def cf(self, xi: np.ndarray, t: float = 1.0) -> np.ndarray:
        ...
        cf_jump_real = interp1d(
            self.xi, np.real(self.jump_cf), kind="linear", fill_value="extrapolate"
        )
        cf_jump_imag = interp1d(
            self.xi, np.imag(self.jump_cf), kind="linear", fill_value="extrapolate"
        )
