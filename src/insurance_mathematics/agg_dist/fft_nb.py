...
class Agg_NbFft(AggregateDistribution):
    ...
    def __init__(
        ...
        """
        frequency_distribution = {"dist": nbinom, "properties": [n, p]}

        super().__init__(
            frequency_distribution, severity_distribution, discretization_step, grid
        )
    ...
    def thin_frequency(self, k: float):
        ...
        """
        q = (
            1 - self.frequency["properties"][1]
        )  # scipy.stats definition uses p as "probability of success" but we need probability of failure
